...
let sessionId = `${Date.now()}-${Math.floor(Math.random() * 1e6)}`;
...
function loadModel(name) {
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      // Remove any old models and add the new one
      scene.children
        .filter(obj => obj.userData?.isModel)
        .forEach(obj => scene.remove(obj));
      ...
      setTimeout(() => {
        const firstAction = Math.floor(Math.random() * 4);
