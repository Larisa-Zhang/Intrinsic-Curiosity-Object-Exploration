...
def main():
    ...
    for c in plot_cols:
        ...
        # ===== NEW: label every 10th block-mean ON the black line + highlight the point =====
        if ANNOTATE_BLOCK_MEANS:
            ...
            # small offset so text sits just above the black line
            ...
            for i in idxs:
                ...
                # 🔥 highlight spot on the black trend line
                plt.scatter(
                    [xi], [yi],
                    s=10,              # size of highlight
                    color="black",
                    alpha=0.9,
                    zorder=5           # draw above other lines
                )

                # number displayed just above that highlighted spot
                plt.text(
                    xi,
                    yi + y_offset,
                    f"{yi:.3g}",
    ...
    pretty_names = [TITLE_NAME_MAP.get(c, c) for c in plot_cols]
    loss_part = " + ".join(pretty_names)

    plt.title(f"{loss_part} in Training (block avg = {AVG_WINDOW} rows)")
